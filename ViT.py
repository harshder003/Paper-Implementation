...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size, patch_size, in_chans = 3, embed_dim = 768):
        ...
        # This is just used for identifying the patches
    ...
    def forward (self, x):
        ...
        """
        x = self.proj(x) # (n _ samples, embed_dim, n_patches ** 0.5, n _ patches ** 0.5)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = qkv.reshape(n_samples, n_tokens, 3, self.n_heads, self.head_dim) # (n_samples, n_patches + 1, 3, self.n_heads, self.head_dim)
        ...
        attn = self.attn_drop(attn) # (n_samples, n_heads, n_patches + 1, n_patches + 1)

        weighted_avg = (attn @ v) # (n_samples, n_heads, n_patches + 1, head_dim)
        weighted_avg = weighted_avg.transpose(1,2) # (n_samples, n_patches + 1, n_heads, head_dim)
        weighted_avg = weighted_avg.flatten(2) # (n_samples, n_heads, n_patches + 1, head_dim)
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.act(x) # (n_samples, n_patches +1, hidden features)
        x = self.drop(x) # (n_samples, n_patches +1, hidden features)
        x = self.fc2(x) # (n_samples, n_patches +1, hidden features)
        x = self.drop(x) # (n_samples, n_patches +1, hidden features)
...
class VisionTransformer(nn.Module):
    ...
    mlp_ratio : float
        Determines the hidden dimension size of MLP module.
    ...
    def forward(self,x):
        ...
        """
        n_samples = x.shape[0]
        x = self.patch_embed(x) # (n_samples, n_patches + 1, embed_dim)
        ...
        for block in self.blocks:
            x = block(x) # (n_samples, n_patches + 1, embed_dim)
